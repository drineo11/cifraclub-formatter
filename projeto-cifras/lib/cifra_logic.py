...
def get_note_index(note):
    # Normalize note
    note = (
        note.replace("Cb", "B").replace("B#", "C").replace("Fb", "E").replace("E#", "F")
    )
                ...
                if segment.get("bold"):  # It's a chord
                    ...
                    for part in parts:
                        ...
                        if part[0] in "ABCDEFG":
                            new_parts.append(
                                transpose_chord(part, semitones, use_flats)
                            )
                        else:
...
def get_cifra_content(url, target_key_index=None):
        ...
        try:
            ...
            for param in fragment.split("&"):
                if param.startswith("key="):
                    ...
                    print(
                        f"DEBUG: Found key in URL: {target_key_index}", file=sys.stderr
                    )
                    break
    ...
    try:
        ...
        if response.status_code == 403:
            raise Exception(
                "Cifra Club está bloqueando requisições. Tente novamente mais tarde."
            )
            ...
            raise Exception("Cifra não encontrada. Verifique a URL.")
        response.raise_for_status()
    except requests.exceptions.Timeout:
        raise Exception("Tempo limite excedido. Verifique sua conexão.")
    ...
    if not pre_content:
        selectors = ["pre", 'div[class*="cifra"]', 'div[id*="cifra"]', ".cifra-content"]
        for selector in selectors:
            pre_content = soup.select_one(selector)
            if pre_content:
                break
    ...
    if not pre_content:
        if os.environ.get("FLASK_ENV") == "development":
            with open("debug_page.html", "w", encoding="utf-8") as f:
                f.write(str(soup))
        raise Exception(
            "Não foi possível encontrar o conteúdo da cifra. O site pode ter mudado."
        )
    ...
    if target_key_index is not None and key:
        try:
            ...
            match = re.search(
                r"forma dos acordes no tom de\s+([A-G][#b]?)", clean_key, re.IGNORECASE
            )
            if match:
                original_key_note = match.group(1).upper()
                print(
                    f"DEBUG: Detected 'forma dos acordes' key: {original_key_note}",
                    file=sys.stderr,
                )
            else:
            ...
            print(
                f"DEBUG: Scraped key: '{key}', Used Key Note: '{original_key_note}'",
                file=sys.stderr,
            )
            ...
            print(
                f"DEBUG: Original Key Index: {original_idx}, Target (Cifra): {target_key_index}",
                file=sys.stderr,
            )
            ...
            if original_idx != -1:
                ...
                if target_key_index in CIFRA_CLUB_KEY_MAP:
                    ...
                    print(
                        f"WARN: Unknown key index {target_key_index}, using formula.",
                        file=sys.stderr,
                    )
                ...
                print(
                    f"DEBUG: Target Chromatic: {target_chromatic_index}, Semitones: {semitones}",
                    file=sys.stderr,
                )
                ...
                # Update key text
                new_key_note = (
                    NOTES_FLAT[target_chromatic_index]
                    if use_flats
                    else NOTES_SHARP[target_chromatic_index]
                )
...
def deduplicate_sections(lines):
    ...
    for section in sections:
        ...
        if header_line:
            ...
            combo_key = f"{header_text}|{body_repr}"

            if combo_key in seen_combinations:
                seen_combinations[combo_key] += 1
                repeat_count = seen_combinations[combo_key]

                new_header = []
                for s in header_line:
                    new_s = s.copy()
                    new_s["italic"] = True
                    new_s["text"] = s["text"].rstrip() + f" {repeat_count}x"
                    new_header.append(new_s)

                final_sections.append({"header": new_header, "body": body_lines})
            else:
                seen_combinations[combo_key] = 1
...
def _estimate_section_height(section_units, font_size):
    line_height = (font_size * 1.2) * 0.3527
    char_width_mm = (font_size * 0.6) * 0.3527
    max_chars_est = int(95 / char_width_mm)
    ...
    for unit in section_units:
        c_len = get_line_length(unit.get("chords", []))
        l_len = get_line_length(unit.get("lyrics", []))
        unit_width = max(c_len, l_len)

        if current_row_units > 0:
            test_len = current_row_len + gap + unit_width
        else:
            test_len = unit_width

        if test_len > max_chars_est:
            height += line_height
            current_row_len = unit_width
            current_row_units = 1
        else:
            current_row_len = test_len
            current_row_units += 1

    if current_row_units > 0:
        height += line_height

    return max(height, line_height)


def split_by_sections(units):
    sections = []
    ...
    for unit in units:
        if unit.get("type") == "header":
            if current_section_units:
                sections.append(current_section_units)
    ...
    font_size_est = 10
    section_heights = [_estimate_section_height(s, font_size_est) for s in sections]

    n = len(sections)
    best_diff = float("inf")
    best_assignments = None

    def try_assignments(idx, height_a, height_b, assignments):
        nonlocal best_diff, best_assignments
        if idx == n:
        ...
        try_assignments(idx + 1, height_a + section_height, height_b, assignments)
        assignments.pop()

        assignments.append("b")
        try_assignments(idx + 1, height_a, height_b + section_height, assignments)
        assignments.pop()

    try_assignments(0, 0, 0, [])
    ...
    if not best_assignments:
        mid = len(units) // 2
        return units[:mid], units[mid:]
    for i, col in enumerate(best_assignments):
        if col == "a":
            col_a.extend(sections[i])
        else:
            col_b.extend(sections[i])

    if not col_b and col_a:
        mid = len(col_a) // 2
        return col_a[:mid], col_a[mid:]
...
def get_content_from_file(filepath):
    ...
    start_index = 0
    if len(raw_lines) > 0 and not is_chord_line(
        [{"text": raw_lines[0], "bold": False}]
    ):
        title = raw_lines[0].strip()
        start_index += 1
    if len(raw_lines) > 1 and not is_chord_line(
        [{"text": raw_lines[1], "bold": False}]
    ):
